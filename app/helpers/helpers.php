 ...
 * Require AFC plugin
 */
function render_image(int $id, $size = 'thumbnail', array $attr = [])
{
  ...
  if (is_array($id)) {
    if (array_key_exists('ID', $id)) {
      return wp_get_attachment_image($id['ID'], $size, false, ["loading" => "lazy", ...$attr]);
    }
    return null;
  }

  return wp_get_attachment_image($id, $size, false, ["loading" => "lazy", ...$attr]);
}
